...
public class SortReview {
	...
	public void mergeSortN(int[] array, boolean type) {
		if (array == null || array.length < 2) {
			return;
		}
		int i, m, n, k1, k2, k;
		int[] tmp = new int[array.length];
		for (int step = 1; step <= array.length / 2+1; step *= 2) {
			for (int s = 0; s < array.length - step ; s += step * 2) {
				i = s;
				m = s + step;
				n = Math.min(s + step * 2, array.length);
				k1 = i;
				k2 = m;
				k = s;
				while (k1 < m && k2 < n) {
					if (array[k1] > array[k2] == type) {
						tmp[k++] = array[k2++];
					} else {
						tmp[k++] = array[k1++];
					}
