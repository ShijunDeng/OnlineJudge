...
class Program3_4 {
	...
	void function() {
		...
		double[][] constantMatrix = new double[2][1];
		for (int i = 0; i < this.getSize(); i++) {
			sum += this.getData()[i][1];
		}
		constantMatrix[0][0] = sum;
		sum = 0d;

		for (int i = 0; i < this.getSize(); i++) {
			sum += this.getData()[i][0];
		}
		coefficientMatrix[1][0] = coefficientMatrix[0][1] = sum;
		sum = 0d;

		coefficientMatrix[0][0] = this.getSize();

		for (int i = 0; i < this.getSize(); i++) {
			sum += this.getData()[i][1] * this.getData()[i][0];
		}
		constantMatrix[1][0] = sum;
		sum = 0d;

		for (int i = 0; i < this.getSize(); i++) {
			sum += Math.pow(this.getData()[i][0], 2);
		}
		coefficientMatrix[1][1] = sum;
		sum = 0d;

		// 开始解方程，确定a,b
		SolveEquation so = new SolveEquation(
				new Matrix(2, 2, coefficientMatrix), new Matrix(2, 1,
						constantMatrix));
		so.solve();
