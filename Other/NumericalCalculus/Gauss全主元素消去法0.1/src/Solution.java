...
public class Solution {
	...
	public void elimination() {// 消元
		...
		for (int k = 1; k < coefficientMatrix.getNumColumns(); k++) {
			...
			// ////////////////////////////////////
			int x1 = 0,x2 = 0;
			for(int i=0;i<coefficientMatrix.getNumColumns();i++)
				if(record[i]==k)
					x1=i;
			for(int j=0;j<coefficientMatrix.getNumColumns();j++)
				if(record[j]==eMax.getColumnsPosition())
					x2=j;
	...
	public void backStrap() {
		...
		for(int i=0;i<coefficientMatrix.getNumRows();i++)
			for(int j=0;j<coefficientMatrix.getNumColumns();j++)
				if(record[j]==i+1){
					System.out.println("---------");
					constantMatrix.exchangeRowValues(i+1, record[j]);
				continue;
				}
	}
