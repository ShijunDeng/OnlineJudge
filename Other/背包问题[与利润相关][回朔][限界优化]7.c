...
void sortT()
{
	...
	for(i=0;i<number&&mark;i++)//for #1
	{
		...
		{	
			if(bonus[j-1]/comms[j-1]<bonus[j]/comms[j])//先计算利润比率再排序更好
			{
				mark=1;
...
double maxBo(double partSum,int sequence)
{
 ...
 double tryKnap(int sequence)
 {
	...
	//maxBo(partSum,sequence) >= maxBonus 注意要取等号 因为sequence为0时 maxBo() maxBonus 都是0 但是试探还是需要进行下去
	if( maxBo(partSum,sequence) >= maxBonus )//不装入第sequence件货物获利可能比当前大 才试探
	{
